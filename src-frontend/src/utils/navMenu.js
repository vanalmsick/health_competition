            ...
            <div className="overflow-x-auto mx-2">
                <div className="flex items-center justify-between">
                    ...
                    <div className="bg-white dark:bg-gray-700 rounded-full shadow-sm w-max mx-auto">
                        <nav className="flex space-x-4 text-sm font-medium text-gray-600 whitespace-nowrap">
                            <Link to='/dashboard'
                            ...
                            {(competitionIsSuccess) ? Object.entries(competitions).map(([_, competition], i) => (
                                <Link key={"key" + competition.id} to={`/competition/${competition.id}`}
                                      className={"px-4 py-2 rounded-full transition-colors " + ((page === `${competition.id}` ? "bg-sky-800 text-white" : "hover:text-light-blue dark:text-white"))}>{competition.name}</Link>
                            )) : null}
                    ...
                    <div className="flex space-x-2 ml-auto">
                        ...
                        <button onClick={() => window.confirm('Not yet implemented')} className="bg-white dark:bg-gray-700 rounded-full shadow-sm w-max p-2">
                ...
                {(showEditCompetitionModal) ? (
                    <CompetitionForm setModalState={setShowEditCompetitionModal} id={showEditCompetitionModal}/>
                ) : null}
            </>
            );
            }